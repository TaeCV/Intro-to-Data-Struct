...
namespace CP {
    ...
    protected:
        ...
        public:
            ...
            hashtable_iterator(BucketIterator bucket, BucketIterator endBucket) : mCurBucketItr(bucket), mEndBucketItr(endBucket) {
                to_next_data();
            }
    ...
    public:
        ...
        // copy constructor
        unordered_map(const unordered_map<KeyT, MappedT, HasherT, EqualT, NextAddressT> &other) : mBuckets(other.mBuckets), mSize(other.mSize),
        ...
        void rehash(size_t n) {
            if (load_factor() <= max_load_factor() && n <= mBuckets.size())
                return;
        ...
        void print() {
            ...
            for (size_t i = 0; i < mBuckets.size(); i++) {
                ...
                if (mBuckets[i].status == 0)
                    std::cout << "[----]";
                else if (mBuckets[i].status == 1)
                    std::cout << "[-- DELETED --]";
                else
                    std::cout << "    [" << mBuckets[i].value.first << ", " << mBuckets[i].value.second << "]";
                std::cout << std::endl;
            }
        }
    };
